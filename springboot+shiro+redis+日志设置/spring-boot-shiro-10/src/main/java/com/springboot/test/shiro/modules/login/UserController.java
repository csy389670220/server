...
public class UserController {
    ...
    public String view(Model model) {
        //测试：在Session中存入数据，会直接更新redis中的session
        SecurityUtils.getSubject().getSession().setAttribute("testName","testName");
        return "这是用户列表页";
    ...
    public String updatePass(Model model) {
        //1.默认管理员修改test用户密码，此处不修改
        String userName="test";
        //2.清除test用户验证缓存
        shiroRealm.removeUserCachedAuthenticationInfo(userName);
        return "修改test用户密码成功";
    ...
    public String updateAuthor(Model model) {
        //1.默认管理员修改test用户授权信息,此处不修改
        String userName="test";
        //2.清除test用户授权缓存
        shiroRealm.removeUserCachedAuthorizationInfo(userName);
        return "修改test用户授权信息成功";
